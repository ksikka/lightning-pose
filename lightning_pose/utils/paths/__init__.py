...
class ResourceUtilImpl(ResourceUtil[KeyType]):
    def __init__(
        self,
        spec: ResourceSpec[KeyType],
        is_multiview: bool,
        get_base_dir: Optional[Callable[[], Optional[Path]]] = None,
    ):
        self._spec = spec
    ...
    def _derive_glob(self) -> tuple[Path, str]:
        ...
        if not template:
            raise ValueError(
                f"No template defined for resource {self._spec.name} (is_multiview={self._is_multiview})."
            )
        ...
        while i < len(s):
            if s[i] == "{":
                ...
                if j == -1:
                    raise ValueError(
                        f"Unclosed placeholder in template for {self._spec.name}: {template}"
                    )
                placeholder = s[i + 1 : j]
        ...
        pattern_rel = (
            glob_pattern[len(start_dir) :]
            if start_dir and glob_pattern.startswith(start_dir)
            else glob_pattern
        )
