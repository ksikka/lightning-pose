...
from lightning_pose.data.keys import VideoFileKey, VideoFrameKey, LabelFileKey
...
def test_video_key_path_conversions(key, path, is_multiview):
    ...
    assert path_util.extract_video_file_key(path) == key
    assert path_util.extract_video_file_key(PurePosixPath(path)) == key
    win_path_str = str(path).replace("/", "\\")
    assert path_util.extract_video_file_key(win_path_str) == key
...
def test_extract_video_file_key_invalid(path, is_multiview):
    ...
    with pytest.raises(PathParseException, match="Could not parse video file path"):
        path_util.extract_video_file_key(path)
    with pytest.raises(PathParseException, match="Could not parse video file path"):
        path_util.extract_video_file_key(PurePosixPath(path))
    win_path_str = str(path).replace("/", "\\")
    with pytest.raises(PathParseException, match="Could not parse video file path"):
        path_util.extract_video_file_key(win_path_str)
...
def test_label_frame_key_path_conversions(key, path, is_multiview):
    ...
    path_util = ProjectPathUtil(project)
    # test key -> path
    assert path_util.get_label_frame_path(key) == Path(path)
    # test path -> key for string, PurePosixPath and windows path string
    assert path_util.extract_label_frame_key(path) == key
    assert path_util.extract_label_frame_key(PurePosixPath(path)) == key
    win_path_str = str(path).replace("/", "\\")
    assert path_util.extract_label_frame_key(win_path_str) == key
...
def test_extract_label_frame_key_invalid(path, is_multiview):
    ...
    with pytest.raises(PathParseException, match="Could not parse label frame path"):
        path_util.extract_label_frame_key(path)
    with pytest.raises(PathParseException, match="Could not parse label frame path"):
        path_util.extract_label_frame_key(PurePosixPath(path))
    win_path_str = str(path).replace("/", "\\")
    with pytest.raises(PathParseException, match="Could not parse label frame path"):
        path_util.extract_label_frame_key(win_path_str)
...
def test_label_file_key_path_conversions(key, view, path, is_multiview):
    ...
    # test path -> key for string, PurePosixPath and windows path string
    assert path_util.extract_label_file_keys(path) == (key, view)
    assert path_util.extract_label_file_keys(PurePosixPath(path)) == (key, view)
    win_path_str = str(path).replace("/", "\\")
    assert path_util.extract_label_file_keys(win_path_str) == (key, view)
...
def test_center_frames_key_path_conversions(key, path, is_multiview):
    ...
    # test path -> key for string, PurePosixPath and windows path string
    assert path_util.extract_center_frames_key(path) == key
    assert path_util.extract_center_frames_key(PurePosixPath(path)) == key
    win_path_str = str(path).replace("/", "\\")
    assert path_util.extract_center_frames_key(win_path_str) == key
...
def test_session_calibration_key_path_conversions(key, path):
    ...
    # test path -> key
    assert path_util.extract_session_key_from_calibration_path(path) == key
...
def test_calibration_backup_key_path_conversions(session_key, time_ns, path):
    ...
    extracted_session_key, extracted_time_ns = (
        path_util.extract_calibration_backup_info(path)
    )
